...
class UNetRecurrent(BaseUNet):
    ...
    def __init__(self, img_chn, ev_chn, out_chn=3, skip_type='sum',
        ...
        super(UNetRecurrent, self).__init__(img_chn, ev_chn, out_chn, skip_type, activation,
    ...
    def forward(self, x, event):
        ...
        for cur_frame in range(0,t):
            e_blocks = [] # initial skip feats for each frame
            e_cur = e[:, cur_frame,:,:,:] # b,c,h,w
            # event encoder
            for i, encoder in enumerate(self.encoders):
                if i==0:
                    e_cur, state = encoder(e_cur, prev_states[i])
                else:
                    # print("e_cur type: {}".format(type(e_cur)))
                    # print("x_blocks[i-1] type: {}".format(type(x_blocks[i-1])))
...
if __name__ == '__main__':
    ...
    model = UNetRecurrent(img_chn=6, ev_chn=2)
    device = 'cuda'
