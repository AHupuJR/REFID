...
class GoProSingleImageEventDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        for blur_path in self.allBlurPath:
            ...
            int_num_name = int(num_name.split('.')[0])
            int_num_start = int_num_name - 5
            int_num_end = int_num_name + 5
            ...
            for i in range(int_num_start, int_num_end):
                basename = '%.6d' % i + '.npz'
                npz_path = blur_path.replace(self.split, self.split+'_event').replace('/blur/', '/')
        ...
        for blur_path in self.allBlurPath:
            sharp_path = blur_path.replace('blur/', 'gt/')
