...
class RuisiEventRecurrentDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(RuisiEventRecurrentDataset, self).__init__()
        self.opt = opt
        ...
        for video in video_list:
            ...
            # events
            eventInputs = [event_frames[i*(self.m+self.n) -1: (i+1)*(self.m+self.n) + self.m] for i in range(1,n_sets)]
    ...
    def __getitem__(self, index):
        ...
        h_lq, w_lq, _ = img_lqs[0].shape # h: 1224    w: 1632
        # print('h:{}'.format(h_lq))
        # print('w:{}'.format(w_lq))
        ...
        if self.one_voxel_flg:
            ...
            for i in range(len(events)):
                y = events[i]['x'].astype(np.float32)[:,np.newaxis]
                x = events[i]['y'].astype(np.float32)[:,np.newaxis]
